...
import {
  FileDownload as DownloadIcon, TableChart as ExcelIcon,
  PictureAsPdf as PdfIcon, DataArray as CsvIcon,
...
export default function ExportButtons() {
  ...
  const handleExcelExport = async () => {
    setLoading(true); handleClose();
    try {
      const filename = await exportToExcel({ metrics: metricsData, chartData, pieData });
  ...
  const handleCSVExport = async () => {
    setLoading(true); handleClose();
    try {
      exportToCSV({ metrics: metricsData, chartData, pieData });
  ...
  const handlePDFReport = async () => {
    setLoading(true); handleClose();
    try {
      const filename = await exportDataToPDF({ metrics: metricsData, chartData, pieData });
